...
from utils.llm import qwen  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Chain-of-thought generation baseline.")
    parser.add_argument("--domain", type=str, default="gender", help="Target domain key.")
    parser.add_argument(
        "--questions",
        nargs="*",
        help="Specific question IDs to evaluate. Defaults to all questions for the domain.",
    )
    parser.add_argument(
        "--question-config",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="CSV containing survey rows.",
    )
    parser.add_argument(
        "--template-dir",
        type=Path,
        default=BASE_DIR / "baseline",
        help="Directory with chain-of-thought templates.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=BASE_DIR / "outputs" / "cot",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Thread pool size for parallel processing.",
    )
...
def evaluate(preds: List[str], truths: List[str]) -> float:
    if not truths:
        return 0.0
    correct = sum(p.lower() == t.lower() for p, t in zip(preds, truths))
...
def main() -> None:
    ...
    template = template_env.get_template("cot_template.md")
